...
def writemincsvgz(f1, ret):
    ...
    try:
        ...
        fn = f1
    with gzip.open(fn+'.gz', "wb", compresslevel=9) as f: # use wb on win, or get more \r \r\n
        if f.tell() == 0:
            f.write(b'\xef\xbb\xbf')
        for i in range(len(ret)):
            f.write((u','.join(ret[i]).rstrip(',') + u'\n').encode('utf-8'))
...
def csvccrgt(s, j):
    ret = [
        ...
        ('_'.join(j['data']['czids'])) if j['data']['czids'][0] != j['data']['czids'][-1] else ('_'.join(j['data']['czids'][:1])),
...
def ccrgtcsv(name, date, cache=1):
    ...
    for i in range(idx, len(c), 1):
        ...
        if key // 10 in [7060,7061,7090,7091,7095,7096]: # S6 S9
            continue
        if key // 100 in [500,501,502, 507,508, 647,648,649, 738,739,747,748,749,750,751,752,753,754,767,768,769,778,779,780,781]: # Dxx D1xx D2xx D7xx D8xx
            continue
        if key // 1000 in [60]: # Cxxx
            continue
        if key // 10000 in [7]: # Cxxx
            continue
